...
class ClipboardTests: XCTestCase {
  ...
  let customType = NSPasteboard.PasteboardType(rawValue: "app.copyagent.ui.mac.ConfidentialType")
  ...
  func testIgnoreApplication() {
    guard let sourceAppBundle = NSWorkspace.shared.frontmostApplication?.bundleIdentifier else {
      XCTFail("Cannot determine frontmost application bundle identifier")
      return
    }
  ...
  func testCopy() {
    ...
    XCTAssertEqual(pasteboard.string(forType: .fromCopyagent), "")
